...
export default function IconLinkCard({ icon, label, value, href, external }: IconLinkCardProps) {
  return (
    <a
      ...
      className="group flex items-center gap-4 rounded-2xl border border-primary/10 bg-white p-6 shadow-sm hover:border-secondary/40 hover:shadow-md transition-all duration-200"
    >
      <span className="flex-shrink-0 text-secondary [&>svg]:h-8 [&>svg]:w-8">{icon}</span>
